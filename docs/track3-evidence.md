...
| **DETERMINE INTERVENTION** | Context + diagnosis → candidate catalogue → ENRV → Lagrangian allocation. Groq does not select. | `tests/recovery/test_candidate_generation_RR-FUNC-020.py`, `tests/recovery/test_valuation_enrv.py` | Inspect opportunity · Recovery Lab · engine selection | `GET /api/opportunity/{id}` |
...
| **MEANINGFUL AI / AGENT** | **Agent loop:** `run_traced_cycle` (detect → ENRV → guard → authorize → execute → measure). **Sandbox overlay:** optional Groq diagnosis/proposal with **no execution authority**. Official engine path: `llm_used=False`, `LLM_OFF`. | `tests/product/test_intelligence.py`, ENRV tests, `test_overview.py` | Workspace AI panel vs PAYVANTA economic decision | `GET /api/product/overview` → `ai` + `track03` (capability_declaration, not proof) |
